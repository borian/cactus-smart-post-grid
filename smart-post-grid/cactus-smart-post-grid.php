...
class cactusSmartPostGrid
{
    ...
    public function spg_parse_shortcode($atts, $content)
    {
        ...
        <section <?php echo($id != '' ? ('id="' . esc_attr($id) . '"') : ''); ?>
                class="cactus-spg scb-heading-<?php echo esc_attr($heading_style); ?> scb-content-<?php echo esc_attr($layout); ?> scb-filter-<?php echo esc_attr($filter_style); ?> scb-id-<?php echo esc_attr($_cactus_scb_count); ?>">
            <div class="section-inner">
                <div class="section-header">
                    <?php if ($title) { ?>
                        <a href="<?php echo esc_url($title_link); ?>"
                           class="btn btn-primary btn-sm <?php echo $icon_class != '' ? $icon_class : ''; ?>">
                    ...
                    <?php } elseif ($filter_style == 2) { ?>
                        <div class="scb-filter-2-btns">
                            <a href="#scb-<?php echo esc_attr($_cactus_scb_count); ?>-all" data-toggle="tab"
                               class="btn btn-grey btn-sm focus"><?php esc_html_e('ALL', "spg"); ?></a>
                            <?php if (is_array($cat_arr) && count($cat_arr)) {
                                foreach ($cat_arr as $cat_id) {
                                    ...
                                    ?>
                                    <a href="#scb-<?php echo esc_attr($_cactus_scb_count . '-' . $cat_id); ?>"
                                       data-toggle="tab" class="btn btn-grey btn-sm"
                                       data-ajax='<?php echo json_encode($atts_ajax); ?>'><?php echo $this_term->name; ?>
                                    </a>
                                <?php }//foreach
                            } ?>
                        </div>
                ...
                <div class="section-content tab-content">
                    <?php if ($filter_style == 2) {
                        echo '<div role="tabpanel" class="tab-pane fade in active" id="scb-' . esc_attr($_cactus_scb_count) . '-all">';
                    } ?>
                        ...
                        if (is_array($cat_arr) && count($cat_arr)) {
                            foreach ($cat_arr as $cat_id) {
                                ...
                                <div role="tabpanel" class="tab-pane fade"
                                     id="scb-<?php echo esc_attr($_cactus_scb_count . '-' . $cat_id); ?>"></div>
                            <?php }//foreach
                        }
                    }//if filter
                    ?>
    ...
    function spg_content_render($atts)
    {
        ...
        $pinned = isset($atts['pinned']) ? $atts['pinned'] : '';

        $page = '1';

        $the_query = $this->spg_shortcode_query($count, $condition, $order, $cats, $tags, $featured, $ids, $page, $post_type, $pinned);
//		$the_query = sort_pinned_posts($the_query);
        ...
        if ($layout == 2) {
            if ($the_query->have_posts()) {
                ...
                while ($the_query->have_posts()) {
                    ...
                    $thumb_size = 'medium';
                    ?>
                    <?php if ($item_count % 4 == 1 && $item_count > 1) {
            ...
            if ($the_query->have_posts()) {
                ...
                while ($the_query->have_posts()) {
                    ...
                    $thumb_size = 'medium';
                    ?>
                    <?php if ($item_count % $divide_number == 1 && $item_count > 1) {
                        echo '</div><div class="row">';
                    } ?>
                    <div class="scb-col-item col-md-<?php echo esc_attr($item_col); ?>">
                        <?php $this->_scb_item_render($thumb_size, $title_in_thumb = 0, $title_class); ?>
            ...
            if ($the_query->have_posts()) {
                ...
                while ($the_query->have_posts()) {
                    ...
                    $thumb_size = 'medium';
                    ?>
                    <?php if ($item_count % 8 == 1 && $item_count > 1) {
                        echo '</div><div class="row">';
                    } ?>
            ...
            if ($the_query->have_posts()) {
                ...
                while ($the_query->have_posts()) {
                    ...
                    $thumb_size = 'medium';
                    ?>
                    <?php if ($item_count % 6 == 1 && $item_count > 1 && $filter_style == 3) {
                        echo '</div><div class="row">';
                    } ?>
                    <div class="scb-col-item col-md-<?php echo esc_attr($item_col); ?>">
            ...
            if ($the_query->have_posts()) {
                ...
                while ($the_query->have_posts()) {
                    ...
                    ?>
                    <?php if ($item_count % 3 == 1 && $item_count > 1) {
                        echo '</div><div class="row">';
                    } ?>
                    <div class="scb-col-item col-sm-<?php echo esc_attr($item_col); ?>">
        ...
        <div class="scb-item <?php echo esc_attr($item_class); ?>">
            <?php if ($item_class == 'scb-masonry-item') { ?>
                <div class="item-content-ontop text-center">
                    <div class="item-label-incontent font-nav">
                        <?php if ($show_category_tag == '1') {
                            $this->spg_get_category($show_class = 0);
                        } ?>
                    </div>
                    <h3 class="item-title"><a href="<?php the_permalink(); ?>"
                                              title="<?php the_title_attribute(); ?>"><?php the_title(); ?></a></h3>
                    <?php if ($show_meta == '1') {
                        $this->spg_sc_item_meta();
                    } ?>
                </div>
            <?php } ?>
            <?php if (has_post_thumbnail()) { ?>
                ...
                <div class="item-thumbnail dark-div">
                    ...
                    <div class="item-label">
                        <?php if ($show_category_tag == '1') {
                            $this->spg_get_category($show_class = 1);
                        } ?>
                    </div>
                </div>
            <?php } //has thumb
            ...
            <div class="item-content">
                <?php if (!$title_in_thumb && $item_class != 'scb-masonry-item') { ?>
                    <h4 class="<?php echo esc_attr($title_class); ?> item-title"><a href="<?php the_permalink(); ?>"
                ...
                <?php if ($item_class == 'scb-masonry-item') { ?>
                    ...
                    <a class="item-readmore-line item-readmore-line-<?php echo esc_attr($category[0]->term_id); ?>-hover"
                       href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>"><i>+</i></a>

                <?php } elseif (@$spg_options['spg_hide_readmore'] != 1) { ?>
                    <a class="item-readmore font-nav" href="<?php the_permalink(); ?>"
                       title="<?php the_title_attribute(); ?>"><i class="fa fa-plus-circle"></i>
                        <span><?php esc_html_e('READ MORE', "spg"); ?></span></a>
                <?php }//if masonry
                ?>
            </div>
    ...
    function spg_shortcode_query($number, $conditions, $sort_by, $categories, $tags, $featured, $ids, $paged, $post_type = 'post', $pinned)
    {
        ...
        if (get_post_type() == 'product' && class_exists(WC_Product)) {
            ...
            <?php if (!$author_link) {
                ...
                <a href="<?php echo get_author_posts_url(get_the_author_meta('ID')); ?>"
                   class="author-url"><?php echo esc_html__('by ', "spg") . get_the_author(); ?></a>
            <?php } ?>
            </span>
            </div>
        <?php }
